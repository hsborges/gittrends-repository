...
async function getRepositoryUpdater(resourceId: string, resources: string[]): Promise<Crawler> {
  return MongoRepository.get(Repository)
    ...
    .then((repo) => {
      ...
      return new RepositoryCrawler(repo._id.toString(), resources as any[], httpClient, {
        writeBatchSize: parseInt(process.env.GT_WRITE_BATCH_SIZE || '500')
      });
    });
}

async function getActorUpdater(resourceId: string): Promise<Crawler> {
  return MongoRepository.get(Actor)
    .collection.findOne({ _id: resourceId })
    .then((user) => {
      if (!user) throw new Error('Actor not found!');
      return new ActorsCrawler(user._id.toString(), httpClient);
    });
}
