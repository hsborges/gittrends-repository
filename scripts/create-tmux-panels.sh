...
tmux new-session -s 'gittrends-panels' -n 'pm2-runtime' \; \
  ...
  send-keys 'yarn workspace @gittrends/service pm2 logs --raw "/repos|users|scheduler/"' C-m \; \
  split-window -v \; \
  send-keys 'yarn workspace @gittrends/service pm2 logs --raw proxy-server' C-m \; \
  split-window -h \; \
  send-keys 'htop' C-m \; \
  split-window -h \; \
  ...
  send-keys 'yarn workspace @gittrends/service pm2 monit' C-m \; \
  select-window -t 1 \;
