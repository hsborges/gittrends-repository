...
tmux new-session -s 'gittrends-monitor' \; \
  ...
  split-window -h \; \
  send-keys 'mongostat -i --authenticationDatabase admin --authenticationMechanism SCRAM-SHA-1 --host ${GT_DATABASE_HOST:-localhost}:${GT_DATABASE_PORT:-27017} $([ -z $GT_DATABASE_USERNAME ] && echo "" || echo " --username ${GT_DATABASE_USERNAME} --password $GT_DATABASE_PASSWORD")' Enter  \; \
